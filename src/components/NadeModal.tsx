...
export default function NadeModal({
  ...
  const id = getYouTubeId(nade.videoUrl);
  const src = id ? youtubeEmbedUrl(id) : "";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      >
        ...
        <div>
          ...
          ) : isYouTube(nade.videoUrl) && getYouTubeId(nade.videoUrl) ? (
            <div className="mt-2 aspect-video rounded overflow-hidden bg-black">
              <iframe
                src={youtubeEmbedUrl(getYouTubeId(nade.videoUrl)!)}
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="mt-6 flex gap-2 justify-end items-center">
            <button
              ...
              className="px-3 py-1 rounded text-sm nm-button border light:border-white border-black"
            >
              Copiar resumen
            </button>
            <button onClick={onClose} className="px-3 py-1 rounded text-sm nm-button border light:border-white border-black">
              Cerrar
            </button>
          </div>
        </div>
  ...
  return (
    <div
      className={`px-2 py-1 rounded-full ${
        theme === "light" ? "bg-neutral-800/60 text-neutral-200" : "bg-neutral-600/60 text-neutral-200"
...
/* helpers */
function humanize(val: any) {
  if (!val && val !== 0) return "—";
  const s = String(val);
  // map crawler-friendly names to readable
